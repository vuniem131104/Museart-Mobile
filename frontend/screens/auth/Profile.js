...
const Profile = () => {
  ...
  console.log(
    "Profile screen - userToken:",
    userInfo ? "exists" : "null",
    "isGuest:",
    isGuest
  );
  ...
  useEffect(() => {
    console.log("Profile userToken check:", accessToken, "isGuest:", isGuest);
  ...
  }, [accessToken, isGuest, navigation]);

  // Fetch profile when accessToken changes (e.g., after login)
  useEffect(() => {
    ...
    if (accessToken && !hasFetchedProfile.current) {
      ...
      fetchUserProfile();
    } else if (accessToken && hasFetchedProfile.current) {
      ...
      console.log(
        "Profile: accessToken present, profile already fetched for this session."
      );
      ...
      if (
        ...
        console.log(
          "Profile: Context userInfo differs from local state, updating local state."
        );
        setUsername(userInfo.username || "");
        setEmail(userInfo.email || "");
      }
    }
  }, [accessToken, userInfo]); // Depend on accessToken AND userInfo now
        ...
        <ScrollView contentContainerStyle={styles.scrollView}>
          ...
          >
            <View style={styles.infoContainer}>
              <Text
                style={[styles.label, { color: isDarkMode ? "#aaa" : "#555" }]}
              >
                Username
              </Text>
              {isEditing ? (
